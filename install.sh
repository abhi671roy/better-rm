...
install() {
    ...
    echo ""
    echo -e "${RED}⚠️  重要免責聲明 / IMPORTANT DISCLAIMER${NC}"
    echo ""
    echo -e "${YELLOW}本工具按「現況」提供，不提供任何保證。${NC}"
    echo -e "${YELLOW}This tool is provided 'AS IS' without any warranty.${NC}"
    echo ""
    echo -e "${YELLOW}本工具不能取代完整的備份策略！${NC}"
