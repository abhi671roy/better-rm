...
install() {
    ...
    echo "================================================"
    echo ""
    echo -e "${RED}⚠️  請注意！別名尚未生效 / ATTENTION! Alias not yet active${NC}"
    echo ""
    echo -e "${YELLOW}請立即執行以下命令以啟用 better-rm：${NC}"
    echo -e "${YELLOW}Please run the following command immediately to activate better-rm:${NC}"
    echo ""
    echo -e "    ${GREEN}source $shell_config${NC}"
    echo ""
    echo -e "${YELLOW}在執行 source 之前，rm 指令仍然是原生的 rm！${NC}"
    echo -e "${YELLOW}Before running source, rm command is still the native rm!${NC}"
    echo ""
    echo "================================================"
    echo ""
    echo "安裝後驗證 (Post-installation verification):"
    echo ""
    echo "1. 先執行上方的 source 命令"
    echo "   First run the source command above"
    echo ""
